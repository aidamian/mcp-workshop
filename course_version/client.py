...
Gemini for tool routing. Logging mirrors the raw_version lifecycle.
"""
...
class GeminiRouter:
  ...
  def route(self, prompt: str) -> ToolCall:
    if self._client is None:
      log_color("[Gemini Router] No API key; using heuristics.", "d", prefix="[debug]")
      return self._fallback.route(prompt)
    try:
      tool_call = self._gemini_route(prompt)
      if self.debug:
  ...
  def _gemini_route(self, prompt: str) -> ToolCall:
    ...
    return ToolCall(tool_name, {key: str(value) for key, value in arguments.items()}, source="gemini")
